<?php
/**
 * SkillBridge - Admin Bulk Questions Import & Export Portal
 */
...
if ($action === 'download_errors') {
    if (isset($_SESSION['admin_question_import_errors'])) {
        download_question_error_report($_SESSION['admin_question_import_errors']);
    } else {
        set_flash_message('danger', 'No error report available. Please upload a file first.');
        redirect(BASE_URL . 'admin/questions-import-export.php');
    }
    exit;
}

// 3. Handle Bulk Export of Questions (Institutional-wide)
if ($action === 'export') {
    $filters = [
    ...
    export_questions_to_csv($db, $filters, $selectedIds, $adminUserId, null); // Pass null for facultyProfileId to allow all
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token()) {
        ...
        if (empty($importRows) || !is_array($importRows)) {
            ...
            try {
                $count = execute_question_import($importRows, $db, $adminUserId);
                set_flash_message('success', "Successfully imported {$count} questions into the institutional assessment system!");
                unset($_SESSION['admin_question_import_errors']);
                redirect(BASE_URL . 'admin/assessments.php');
            } catch (Throwable $e) {
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($ext, ['csv', 'txt', 'xlsx'])) {
                ...
                if (!$parsed['success']) {
                    ...
                    // Validate Questions (pass null as facultyProfileId to allow importing to all assessments)
                    $previewData = validate_question_import_rows($parsed['rows'], $db, null);
                    
                    // Save invalid rows to session for error report download
                    $invalidRows = array_filter($previewData['rows'], fn($x) => !$x['is_valid']);
                    $_SESSION['admin_question_import_errors'] = $invalidRows;
                }
            }
        }
    }
...
// Fetch all assessments in the system to populate the export filters
$assessmentsList = $db->fetchAll("SELECT id, title FROM assessments ORDER BY title ASC");

$pageTitle = "Institutional Bulk Question Import & Export – Admin Portal";
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            ...
            <p class="text-muted small mb-0">System-wide import and export operations for assessment question banks</p>
        </div>
    </div>
    <a href="<?= BASE_URL ?>admin/assessments.php" class="btn btn-outline-secondary rounded-pill px-3 py-2 small fw-semibold">
        <i class="bi bi-clipboard-check"></i> Assessments Oversight
    </a>
</div>
...
<div class="card border-0 shadow-sm rounded-4 mb-4">
    <div class="card-header bg-white border-bottom-0 p-3">
        <ul class="nav nav-pills nav-fill gap-2" id="importExportTabs">
            <li class="nav-item">
                <a class="nav-link rounded-pill py-2.5 fw-semibold" href="<?= BASE_URL ?>admin/import-export.php?type=students">
                    <i class="bi bi-mortarboard me-1.5"></i> Students Import & Export
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link rounded-pill py-2.5 fw-semibold" href="<?= BASE_URL ?>admin/import-export.php?type=faculty">
                    <i class="bi bi-person-badge me-1.5"></i> Faculty Members Import & Export
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link rounded-pill py-2.5 fw-semibold active" href="<?= BASE_URL ?>admin/questions-import-export.php">
    ...
    <div class="saas-card overflow-hidden mb-4 border-primary shadow-sm">
        ...
        <div class="card-body p-4">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 pt-3 border-top">
                <a href="<?= BASE_URL ?>admin/questions-import-export.php" class="btn btn-outline-secondary rounded-pill px-4">
                    <i class="bi bi-arrow-left me-1"></i> Cancel & Re-upload
                </a>

                <div class="d-flex gap-2">
                    <?php if ($previewData['valid_count'] > 0): ?>
                        <form action="<?= BASE_URL ?>admin/questions-import-export.php" method="POST" onsubmit="showProgressBar()">
                            <?= csrf_field() ?>
    ...
    <div class="row g-4 mb-4">
            ...
            <div class="saas-card h-100 overflow-hidden">
                <div class="saas-card-header py-3 px-4">
                    <h5 class="fw-bold mb-0" style="color: var(--text-heading);">
                        <i class="bi bi-file-earmark-arrow-up text-primary me-2"></i>Bulk Import Questions (Admin)
                    </h5>
                ...
                <div class="card-body p-4 d-flex flex-column justify-content-between">
                    <form action="<?= BASE_URL ?>admin/questions-import-export.php" method="POST" enctype="multipart/form-data" id="uploadForm">
                        <?= csrf_field() ?>
                        ...
                        <div class="p-3 bg-light rounded-3 small text-muted">
                            ...
                            <ul class="mb-0 ps-3">
                                ...
                                <li>Permissions: Admin can import into any assessment across all creators.</li>
                            </ul>
                        </div>
                    </form>
                </div>
        ...
        <div class="col-lg-6">
            <div class="saas-card h-100 overflow-hidden">
                ...
                <div class="card-body p-4 d-flex flex-column justify-content-between">
                    <form action="<?= BASE_URL ?>admin/questions-import-export.php" method="POST">
                        <?= csrf_field() ?>
                        <input type="hidden" name="action" value="export">

                        <p class="text-muted small mb-3">Filter and export all system-wide assessment questions to an Excel-compatible CSV file.</p>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="assessment_id" class="saas-form-select w-100">
                                <option value="all">All Institutional Assessments</option>
