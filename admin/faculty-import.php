<?php
/**
 * SkillBridge - Admin Faculty Import Portal
 * Context-specific Faculty CSV Import portal.
 */
    ...
    if (!verify_csrf_token()) {
        ...
        if (empty($importRows) || !is_array($importRows)) {
            ...
            try {
                ...
                set_flash_message('success', "Successfully imported {$count} faculty records!");
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($ext, ['csv', 'txt'])) {
                ...
                if (!$parsed['success']) {
                    ...
                    $previewData['entity_type'] = 'faculty';
                }
            }
        }
    }
...
$pageTitle = "Faculty CSV Import - Admin Portal";
include __DIR__ . '/../includes/header.php';
?>

<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
    <div>
        <h3 class="fw-bold mb-1 text-dark"><i class="bi bi-cloud-arrow-up text-info me-2"></i>Faculty Accounts CSV Import</h3>
        <p class="text-muted small mb-0">Import and configure faculty accounts directly using a standardized CSV template</p>
    </div>
    <a href="<?= BASE_URL ?>admin/faculty.php" class="btn btn-outline-secondary rounded-pill px-3 py-1.5 small fw-semibold">
...
<div class="row g-4">
    <div class="col-lg-8">
        <div class="saas-card p-5 text-center border-dashed position-relative" id="dropZone" style="border: 2px dashed var(--info); background: rgba(13, 202, 240, 0.02); border-radius: 1rem;">
            <form action="<?= BASE_URL ?>admin/faculty-import.php" method="POST" enctype="multipart/form-data" id="uploadForm">
                <?= csrf_field() ?>
                <div class="mb-3 text-info" style="font-size: 3rem;"><i class="bi bi-file-earmark-arrow-up"></i></div>
                <h5 class="fw-bold text-dark mb-2">Drag and drop your Faculty CSV here</h5>
    ...
    <div class="col-lg-4">
        <div class="saas-card p-4">
            <h5 class="fw-bold text-dark mb-3"><i class="bi bi-info-circle text-info me-1"></i>Instructions</h5>
            <ol class="small text-secondary ps-3 mb-4">
                <li class="mb-2">Download the official faculty CSV template using the button below.</li>
                <li class="mb-2">Fill in your faculty records (First Name, Last Name, Email, Department, Designation, Employee Code, etc.).</li>
            ...
            <a href="<?= BASE_URL ?>admin/faculty-import.php?action=download_template" class="btn btn-outline-info w-100 rounded-pill fw-semibold py-2 small">
...
<div class="saas-card mb-4 p-4">
    <div class="border-bottom pb-3 mb-4 d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2">
            <a href="<?= BASE_URL ?>admin/faculty-import.php" class="btn btn-light rounded-pill px-4 py-1.5 small fw-semibold">Cancel</a>
            <?php if ($previewData['valid_count'] > 0): ?>
                <form action="<?= BASE_URL ?>admin/faculty-import.php" method="POST" class="d-inline">
                    <?= csrf_field() ?>
                    ...
                    <button type="submit" class="btn btn-success rounded-pill px-4 py-1.5 small fw-semibold">
                        Confirm Import (<?= $previewData['valid_count'] ?> Faculty Members)
                    </button>
                </form>
            <?php endif; ?>
        </div>
...
if (dropZone) {
    dropZone.addEventListener('dragover', (e) => {
        e.preventDefault();
        dropZone.style.background = "rgba(13, 202, 240, 0.08)";
        dropZone.style.borderColor = "var(--info)";
    });
    dropZone.addEventListener('dragleave', () => {
        dropZone.style.background = "rgba(13, 202, 240, 0.02)";
        dropZone.style.borderColor = "var(--info-light)";
    });
}
</script>
