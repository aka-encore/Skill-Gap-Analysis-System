<?php
/**
 * SkillBridge - Admin Question Import Page
...
if (!$qb) {
    ...
    redirect(BASE_URL . 'admin/question-bank.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token()) {
        ...
        if (empty($importRows) || !is_array($importRows)) {
            ...
            try {
                $count = execute_qbank_question_import($importRows, $db, $qbId, $adminUserId);
...
$pageTitle = "Import Questions - Admin Portal";
...
<div class="row g-4">
    <div class="col-lg-8">
        <div class="saas-card p-5 text-center border-dashed position-relative" id="dropZone" style="border: 2px dashed var(--success); background: rgba(25, 135, 84, 0.02); border-radius: 1rem;">
            <form action="<?= BASE_URL ?>admin/question-import.php" method="POST" enctype="multipart/form-data" id="uploadForm" onsubmit="showUploadProgress()">
                <?= csrf_field() ?>
    ...
    <div class="col-lg-4">
        <div class="saas-card p-4">
            ...
            <a href="<?= BASE_URL ?>admin/question-import.php?action=download_template" class="btn btn-outline-success w-100 rounded-pill fw-semibold py-2 small">
...
<div class="saas-card mb-4 p-4">
    <div class="border-bottom pb-3 mb-4 d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2">
            <a href="<?= BASE_URL ?>admin/question-import.php?qb_id=<?= $qbId ?>" class="btn btn-light rounded-pill px-4 py-1.5 small fw-semibold">Cancel</a>
            <?php if ($previewData['valid_count'] > 0): ?>
                <form action="<?= BASE_URL ?>admin/question-import.php" method="POST" class="d-inline">
                    <?= csrf_field() ?>
                    <input type="hidden" name="qb_id" value="<?= $qbId ?>">
