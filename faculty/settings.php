 ...
 * Aligned 1-to-1 with Student Settings design language.
 */
...
// Fetch authenticated faculty & user record first
$faculty = $db->fetch(
    "SELECT f.*, u.username, u.email, u.role, u.password as user_pw, u.created_at as user_created 
     FROM faculty f 
...
// Initialize default notification preferences in session/DB if not set
if (!isset($_SESSION['faculty_notif_prefs'])) {
    $_SESSION['faculty_notif_prefs'] = [
        'notif_assessment' => (int)($faculty['notif_assessment'] ?? 1),
        'notif_submission' => (int)($faculty['notif_submission'] ?? 1)
    ];
}
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && isset($_POST['update_preferences'])) {
    ...
    $newSubm = isset($_POST['notif_submission']) ? 1 : 0;

    $currAss  = $_SESSION['faculty_notif_prefs']['notif_assessment'] ?? 1;
    $currSubm = $_SESSION['faculty_notif_prefs']['notif_submission'] ?? 1;

    if ($newAss === $currAss && $newSubm === $currSubm) {
        set_flash_message('info', 'No changes were made to your notification settings.');
    } else {
        $_SESSION['faculty_notif_prefs'] = [
            'notif_assessment' => $newAss,
            'notif_submission' => $newSubm
        ];
...
$pageTitle = "Faculty Account & Platform Settings - SkillBridge";
  ...
  <div class="row g-4">
    ...
    <div class="col-lg-8">
      ...
      <div class="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-white" id="change-password">
        ...
        <p class="text-muted small mb-4">Update your account password to keep your faculty portal records secure.</p>

        <form action="<?= BASE_URL ?>faculty/settings.php" method="POST" class="d-flex flex-column gap-3" id="changePwForm">
      ...
      <div class="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-white">
        ...
        <p class="text-muted small mb-4">Manage your alert preferences for assessment publishes and student submission updates.</p>

        <form action="<?= BASE_URL ?>faculty/settings.php" method="POST" class="d-flex flex-column gap-3">
          <input type="hidden" name="update_preferences" value="1">
          
          <div class="p-3 bg-light rounded-3 border">
            <div class="form-check form-switch mb-2">
              <input class="form-check-input" type="checkbox" name="notif_assessment" id="notifAssessment" value="1" <?= ($_SESSION['faculty_notif_prefs']['notif_assessment'] ?? 1) ? 'checked' : '' ?>>
              <label class="form-check-input-label fw-bold text-dark small" for="notifAssessment">Assessment & Question Pool Alerts</label>
            </div>
            <div class="text-muted" style="font-size: 11px;">Receive notifications when question banks or assessments are published or updated.</div>
          </div>

          <div class="p-3 bg-light rounded-3 border">
            <div class="form-check form-switch mb-2">
              <input class="form-check-input" type="checkbox" name="notif_submission" id="notifSubmission" value="1" <?= ($_SESSION['faculty_notif_prefs']['notif_submission'] ?? 1) ? 'checked' : '' ?>>
              <label class="form-check-input-label fw-bold text-dark small" for="notifSubmission">Student Submission Notifications</label>
            </div>
            <div class="text-muted" style="font-size: 11px;">Receive alerts when students complete assessment attempts in your department.</div>
          </div>
    ...
    <div class="col-lg-4">
      ...
      <div class="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-white">
        ...
        <div class="d-flex flex-column gap-2 small">
          ...
          <div class="d-flex justify-content-between py-1.5 border-bottom">
            ...
            <span class="badge bg-success-subtle text-success border text-uppercase fw-semibold"><?= htmlspecialchars($faculty['role'] ?? 'faculty') ?></span>
          </div>
          <div class="d-flex justify-content-between py-1.5 border-bottom">
            <span class="text-muted">Employee Code</span>
            <span class="fw-bold text-dark"><?= htmlspecialchars($faculty['employee_code'] ?? 'FAC-101') ?></span>
          </div>
          <div class="d-flex justify-content-between py-1.5 border-bottom">
            <span class="text-muted">Department</span>
            <span class="fw-bold text-dark"><?= htmlspecialchars($faculty['department'] ?? 'Computer Science') ?></span>
          </div>
          <?php if (!empty($faculty['designation'])): ?>
          <div class="d-flex justify-content-between py-1.5 border-bottom">
            <span class="text-muted">Designation</span>
            <span class="fw-bold text-dark"><?= htmlspecialchars($faculty['designation']) ?></span>
          </div>
          <?php endif; ?>
          <div class="d-flex justify-content-between py-1.5 border-bottom">
            <span class="text-muted">Username</span>
            <span class="fw-bold text-dark"><?= htmlspecialchars($faculty['username'] ?? '') ?></span>
          </div>
          <div class="d-flex justify-content-between py-1.5">
            <span class="text-muted">Email Address</span>
            <span class="fw-semibold text-dark"><?= htmlspecialchars($faculty['email'] ?? '') ?></span>
          </div>
        </div>
      </div>
      ...
      <div class="card border-0 shadow-sm rounded-4 p-4 bg-danger-subtle border-danger-subtle">
        ...
        <p class="text-muted small mb-3">Sign out of your SkillBridge faculty session on this browser.</p>
