...
// Fetch all notifications for student ordered newest to oldest
...
.notif-icon-feedback     { background: #CFE2FF; color: #084298; }
.notif-icon-certificate  { background: #E0CFFC; color: #3D0A91; }
.notif-icon-course       { background: #E2D9F3; color: #491217; }
.notif-icon-profile      { background: #D2F4EA; color: #0A3622; }
...
<div class="dash-content">
  ...
  <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
    <div>
      <h3 class="fw-bold text-dark mb-1"><i class="fa-solid fa-bell text-primary me-2"></i>Notification Center</h3>
      <p class="text-muted small mb-0">Stay updated on your assessments, feedback, badges, course recommendations, and system alerts.</p>
    </div>
  ...
  <div class="card border-0 shadow-sm rounded-4 p-3 mb-4 bg-white">
    ...
    <div class="row g-3 align-items-center mb-3">
      <div class="col-md-7 col-lg-8">
        <div class="input-group">
          ...
          <input type="text" id="notifSearchInput" class="form-control bg-light border-start-0 rounded-end-pill py-2 text-dark small" placeholder="Search by title, description, or notification type...">
        </div>
      </div>
    ...
    <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3 pt-2 border-top">
      <div class="d-flex align-items-center gap-2 overflow-auto pb-2 pb-lg-0" id="filterTabsContainer" style="scrollbar-width: thin;">
        ...
        <button class="notif-filter-btn" data-filter="achievement" onclick="setNotifFilter('achievement', this)">Achievements</button>
        <button class="notif-filter-btn" data-filter="feedback" onclick="setNotifFilter('feedback', this)">Feedback</button>
        <button class="notif-filter-btn" data-filter="certificate" onclick="setNotifFilter('certificate', this)">Certificates</button>
  ...
  <div id="notifsListContainer" class="d-flex flex-column gap-3">
    <?php foreach ($notifications as $n): 
        ...
        if (str_contains(strtolower($title), 'urgent') || str_contains(strtolower($title), 'high') || str_contains(strtolower($title), 'critical')) {
            $priority = 'high';
        } elseif (str_contains(strtolower($title), 'important') || str_contains(strtolower($title), 'remind')) {
            $priority = 'medium';
        }
        ...
        $iconBoxClass = 'notif-icon-' . (in_array($type, ['assessment','announcement','achievement','feedback','certificate','course','profile','system','reminder']) ? $type : 'general');
        ...
        <div class="d-flex align-items-start gap-3">
          ...
          <div class="flex-grow-1 min-w-0">
            ...
            <div class="d-flex align-items-center gap-2">
              <span class="badge bg-light text-secondary border rounded-2 text-uppercase fw-semibold" style="font-size: 10px; letter-spacing: 0.5px;"><?= ucfirst($type) ?></span>
  ...
  <div id="notifEmptyState" class="card border-0 shadow-sm rounded-4 p-5 text-center my-4 bg-white" style="display: none;">
    ...
    <div>
      <a href="<?= BASE_URL ?>student/dashboard.php" class="btn btn-primary rounded-pill px-4 py-2 small fw-semibold">
...
function handleCardClick(id, redirectUrl, isAnnouncement, annId, event) {
    if (isAnnouncement && annId > 0 && typeof window.openAnnouncementModal === 'function') {
        ...
        fetch(BASE_URL + 'api/notifications_action.php', {
            ...
            body: 'action=mark_read&id=' + id + '&csrf_token=<?= $_SESSION['csrf_token'] ?>'
        }).then(() => {
...
function toggleNotifReadStatus(id, event) {
    ...
    fetch(BASE_URL + 'api/notifications_action.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: `action=${nextAction}&id=${id}&csrf_token=<?= $_SESSION['csrf_token'] ?>`
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
...
function deleteSingleNotifAjax(id, event) {
    ...
    fetch(BASE_URL + 'api/notifications_action.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'action=delete&id=' + id + '&csrf_token=<?= $_SESSION['csrf_token'] ?>'
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
...
function markAllNotifsReadAjax() {
    fetch(BASE_URL + 'api/notifications_action.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'action=mark_all_read&csrf_token=<?= $_SESSION['csrf_token'] ?>'
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
...
function clearAllNotifsAjax() {
    ...
    fetch(BASE_URL + 'api/notifications_action.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'action=clear_all&csrf_token=<?= $_SESSION['csrf_token'] ?>'
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', window.debounce(filterAndSortNotifs, 250));
    }
});
</script>
